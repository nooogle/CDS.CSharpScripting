...
namespace CDS.CSharpScript.ScintillaEditor
{
    ...
    public partial class CodeEditor : UserControl
    {
        ...
        private Core.Editor.Workspace workspace;
        ...
        {
            CDSInitialize(
                namespaceTypes: default,
                referenceTypes: default,
            ...
            Type globalsType)
        {
            ...
            namespaceTypes = namespaceTypes ?? new Type[0];
            referenceTypes = referenceTypes ?? new Type[0];

            var namespaceTypesIncludingGlobalsType = GenerateAllNamespaceTypes(namespaceTypes, globalsType);

            var additionalAssemblies =
                GenerateAllReferenceTypes(referenceTypes, globalsType)
                .Concat(namespaceTypesIncludingGlobalsType.Select(r => r.Assembly))
                .Distinct();

            workspace = new Core.Editor.Workspace(
                namespaceTypes: namespaceTypesIncludingGlobalsType,
        ...
        private static IEnumerable<Assembly> GenerateAllReferenceTypes(IEnumerable<Type> referenceTypes, Type globalsType)
        {
        ...
        private async void RunAutoComplete()
        {
            ...
            try
            {
                var autoShowList = await GetAutoCompletions();
        ...
        private async Task<IEnumerable<string>> GetAutoCompletions()
        {
            var completions = await Core.Editor.Completions.GetCompletions(
                workspace: workspace,
                script: CDSScript,
                caretPosition: CDSSelectionStart,
        ...
        private void editor_KeyDown(object sender, KeyEventArgs e)
        {
            if (workspace == null)
            {
                throw new InvalidOperationException($"{nameof(CDSInitialize)} must be called before enabling and using the editor");
            }
